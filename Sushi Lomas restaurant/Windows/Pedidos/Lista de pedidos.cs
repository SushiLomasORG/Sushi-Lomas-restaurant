        ...
        void list()
        {
            try
            {
                string consulta = @"select 
                                     ...
                                     left join Pedido_domicilio pd on pd.id_pedido = p.id 
                                     left join estado_pedido ep on ep.id = p.estado
                                     left join tipo_pedido tp on tp.id = p.lugar where cast(p.fecha as date) = cast(getdate() as date) and telefono like '%" + txt_filtrar.Text + "%'"
                ;
        ...
        void listxTipo()
        {
            try
            {
                string consulta = $@"select 
                                      ...
                                      left join Pedido_domicilio pd on pd.id_pedido = p.id 
                                      left join estado_pedido ep on ep.id = p.estado
                                      left join tipo_pedido tp on tp.id = p.lugar where cast(p.fecha as date) = cast(getdate() as date) and p.estado = {tipo} and telefono like '%" + txt_filtrar.Text + "%'"
                ;
        ...
        void ver_detalle()
        {
            try
            {
                string consulta = $@"SELECT 
                                        ...
                                        dp.detalle AS Detalles,
                                        p.n_pedido AS Pedido
                                     FROM Detalle_pedido dp
                                     JOIN Articulo a ON dp.articulo = a.id 
                                     JOIN Pedido p ON dp.id_pedido = p.id where id_pedido = {id_pedido};"
                ;
